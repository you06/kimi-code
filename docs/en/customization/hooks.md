...
| `PostCompact` | `manual` or `auto` | `trigger`, `summary`, `compacted_count`, `tokens_before`, `tokens_after`, `compacted_messages`, `estimated_token_count` (deprecated alias of `tokens_after`) | Fires asynchronously after context compaction is successfully written. `compacted_messages` is the raw `[{role, content}]` prefix that was just replaced by the summary — exactly what `tokens_after` measures. Blocking results do not change the main flow |
