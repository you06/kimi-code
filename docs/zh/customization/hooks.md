...
| `PostCompact` | `manual` 或 `auto` | `trigger`、`summary`、`compacted_count`、`tokens_before`、`tokens_after`、`compacted_messages`、`estimated_token_count`（`tokens_after` 的废弃别名） | 上下文压缩成功写入后异步触发。`compacted_messages` 是被 summary 替换掉的原始 `[{role, content}]` 前缀，对应 `tokens_after` 度量的范围。阻断结果不会改变主流程 |
